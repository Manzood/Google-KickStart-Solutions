...
// problem seems to have far too many parts, maybe I'll come back to it later
// when I've learnt how to use the individual techniques
...
// store the value in the answer array after taking modular inverse? Or
...
vector<bool> visited;
void dfs(int node, vector<vector<int>>& adj, vector<int>& parent,
         vector<int>& prob, vector<int>& prob_not, vector<int>& a,
         vector<int>& b) {
    ...
    if (node != 0) {
        prob[node] =
            prob[parent[node]] * a[node] + prob_not[parent[node]] * b[node];
...
void testcase() {
    ...
    // convert it into modular inverse, and keep using that for each
    // intermediate step as well recursive function to find the probability
    // every time??
}
